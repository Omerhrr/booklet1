...
def profit_loss():
    ...
    try:
        response = requests.get(f'{API_BASE}/reports/profit-loss', params={'start_date': start_date, 'end_date': end_date, 'compare_previous': compare}, headers=get_headers())
...
def balance_sheet():
    ...
    try:
        response = requests.get(f'{API_BASE}/reports/balance-sheet', params={'as_of_date': as_of_date}, headers=get_headers())
        report = response.json() if response.status_code == 200 else {}
    except:
        report = {}
...
def trial_balance():
    as_of_date = request.args.get('as_of_date', datetime.now().strftime('%Y-%m-%d'))
    try:
        response = requests.get(f'{API_BASE}/reports/trial-balance', params={'as_of_date': as_of_date}, headers=get_headers())
        report = response.json() if response.status_code == 200 else {}
    except:
        report = {}
...
def aging():
    ...
    try:
        if report_type == 'payables':
            response = requests.get(f'{API_BASE}/reports/payables-aging', headers=get_headers())
        else:
            response = requests.get(f'{API_BASE}/reports/receivables-aging', headers=get_headers())
        report = response.json() if response.status_code == 200 else {}
    except:
        report = {}
...
def sales_summary():
    ...
    try:
        response = requests.get(f'{API_BASE}/reports/sales-summary', params={'start_date': start_date, 'end_date': end_date, 'group_by': group_by}, headers=get_headers())
        report = response.json() if response.status_code == 200 else {}
    except:
        report = {}
...
def cash_flow():
    ...
    end_date = request.args.get('end_date', datetime.now().strftime('%Y-%m-%d'))
    try:
        response = requests.get(f'{API_BASE}/reports/cash-flow', params={'start_date': start_date, 'end_date': end_date}, headers=get_headers())
        report = response.json() if response.status_code == 200 else {}
    except:
        report = {}
